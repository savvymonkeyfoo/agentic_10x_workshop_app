...
import { DeleteConfirmationModal } from "@/components/ui/DeleteConfirmationModal"; // NEW IMPORT
...
export function OpportunityModal({ card, isOpen, onClose, onSave, onEnrich, onDelete }: OpportunityModalProps) {
    ...
    return (
        <>
            ...
            {/* NEW: STYLED CONFIRMATION MODAL */}
            <DeleteConfirmationModal
                isOpen={isDeleteModalOpen}
                onClose={() => setIsDeleteModalOpen(false)}
                onConfirm={() => {
                    if (onDelete && localCard) onDelete(localCard);
                    setIsDeleteModalOpen(false);
                }}
                title="Delete Opportunity"
                description="Are you sure you want to remove this opportunity? This action cannot be undone and will remove it from all views."
            />
        </>
    );
}
