...
async function main() {
    ...
    if (!workshop) {
        console.error(`❌ Could not find "Zurich Insurance Australia"`);
        return;
    }

    console.log(`✅ Found Target: ${workshop.id}`);
    ...
    const opportunities = [
        {
            ...
            // CRASH FIX: DFV Assessment with correct nesting
            dfvAssessment: {
            ...
            // VALUE PROP (Mapping to strategicRationale or JSON if columns exist)
    ...
    // 3. Inject Data (Delete old opps first to avoid duplicates)
    ...
    for (const op of opportunities) {
        await prisma.opportunity.create({
            data: {
                ...
                impactedSystems: [] // Added default for required field
                // Prisma handles Json type automatically if passed as object.
            }
        });
    }

    console.log(`🚀 Successfully fully restored ${workshop.clientName}`);
}
